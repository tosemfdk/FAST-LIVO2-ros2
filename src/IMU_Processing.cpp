...
{
  RCLCPP_WARN(fastLivoLogger(), "Reset ImuProcess");
  mean_acc = V3D(0, 0, -1.0);
  mean_gyr = V3D(0, 0, 0);
  angvel_last = Zero3d;
  imu_need_init = true;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
  ...
  RCLCPP_INFO(fastLivoLogger(), "IMU Initializing: %.1f %%", double(N) / MAX_INI_COUNT * 100);
  V3D cur_acc, cur_gyr;
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  if (!imu_time_init)
  {
    // imu_time = stampToSec(v_imu.front()->header.stamp) - first_lidar_time;
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      // imu_time = stampToSec(head->header.stamp) - first_lidar_time;
      ...
      // cout<<setw(20)<<"offset_t: "<<offs_t<<"stampToSec(tail->header.stamp):
      // "<<stampToSec(tail->header.stamp)<<endl; printf("[ LIO Propagation ]
      // offs_t: %lf \n", offs_t);
...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      imu_need_init = false;
      RCLCPP_INFO(fastLivoLogger(), "IMU Initials: Gravity: %.4f %.4f %.4f %.4f; acc covarience: "
               ...
               cov_gyr[2]);
      RCLCPP_INFO(fastLivoLogger(), "IMU Initials: ba covarience: %.8f %.8f %.8f; bg covarience: "
               "%.8f %.8f %.8f",
