...
{
  voxel_config.is_pub_plane_map_ = declareAndGetParam<bool>(node, "publish/pub_plane_en", false);
  voxel_config.max_layer_ = declareAndGetParam<int>(node, "lio/max_layer", 1);
  voxel_config.max_voxel_size_ = declareAndGetParam<double>(node, "lio/voxel_size", 0.5);
  voxel_config.planner_threshold_ = declareAndGetParam<double>(node, "lio/min_eigen_value", 0.01);
  voxel_config.sigma_num_ = declareAndGetParam<double>(node, "lio/sigma_num", 3.0);
  voxel_config.beam_err_ = declareAndGetParam<double>(node, "lio/beam_err", 0.02);
  voxel_config.dept_err_ = declareAndGetParam<double>(node, "lio/dept_err", 0.05);
  voxel_config.layer_init_num_ = declareAndGetParam<std::vector<int>>(node, "lio/layer_init_num", std::vector<int>{5, 5, 5, 5, 5});
  voxel_config.max_points_num_ = declareAndGetParam<int>(node, "lio/max_points_num", 50);
  voxel_config.max_iterations_ = declareAndGetParam<int>(node, "lio/max_iterations", 5);
  voxel_config.map_sliding_en = declareAndGetParam<bool>(node, "local_map/map_sliding_en", false);
  voxel_config.half_map_size = declareAndGetParam<int>(node, "local_map/half_map_size", 100);
  voxel_config.sliding_thresh = declareAndGetParam<double>(node, "local_map/sliding_thresh", 8.0);
}
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat)
{
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    if (!EKF_stop_flg && (rematch_num >= 2 || (iterCount == config_setting_.max_iterations_ - 1)))
    {
      ...
      tf2::Quaternion tf_q; tf_q.setRPY(euler_cur(0), euler_cur(1), euler_cur(2)); geoQuat_ = tf2::toMsg(tf_q);
